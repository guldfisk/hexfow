...
class Creature:
    ...
    def from_str(cls, s: str) -> Creature:
        return cls(s, Price(int(match.group(1)), match.group(2)) if (match := re.match(r"^[^{]*\{(\d+)([a-z]*)}", s)) else Price(0, ''))
...
def sort_creatures():
    result = '\n\n'.join(c.serialize() for c in sorted(load_creatures(), key=lambda c: c.cost))
